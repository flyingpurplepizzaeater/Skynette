...
must_haves:
  truths:
    ...
    - "User sees available Ollama models refresh on tab mount via did_mount hook"
...
Purpose: Give users clear visibility into Ollama connection status and available models, with auto-refresh on tab mount and manual refresh capability.
...
<task type="auto">
  <name>Task 3: Add Ollama status UI with did_mount auto-refresh</name>
  <files>src/ui/views/ai_hub/providers.py</files>
  <action>
Add Ollama status indicator, refresh button, and automatic refresh on tab mount.
...
**CRITICAL: Add did_mount hook for auto-refresh on tab load:**
```python
def did_mount(self):
    """Called when component is mounted. Trigger Ollama status refresh."""
    # Schedule the async refresh
    import asyncio
    asyncio.create_task(self._on_refresh_ollama(None))
```

If the component uses a different lifecycle pattern (e.g., Flet's UserControl), implement accordingly:
- For UserControl: Override `def did_mount(self)` method
- For function-based components: Call refresh in the initial build if state shows no previous refresh

This ensures models refresh automatically when the user navigates to the Providers tab.
  </action>
  <verify>
Manual verification: Run the app and navigate to AI Hub -> Providers tab.
- On first load: Should automatically trigger refresh (show "Refreshing..." briefly)
...
- Click refresh: Should show "Refreshing..." briefly then update status
  ...
  <done>Providers tab shows Ollama connection status with model count, refresh button, and auto-refreshes on tab mount via did_mount</done>
</task>
...
- Providers tab auto-refreshes Ollama status on mount via did_mount hook
