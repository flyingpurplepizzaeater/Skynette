...
must_haves:
  truths:
    - "User sees usage meter when approaching rate limits (80%+ quota used)"
...
Purpose: Users should never be surprised by rate limit errors or lose content when streams fail. They see usage meters when approaching limits and get clear recovery paths when things go wrong.
...
- No auto-retry - show error with manual Retry button (UI handles this)
  </action>
  <verify>
