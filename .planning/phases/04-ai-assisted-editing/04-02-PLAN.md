...
must_haves:
  ...
  key_links:
    - from: "chat_panel.py"
      ...
      via: "async streaming call for chat responses"
      pattern: "gateway\\.chat_stream"
    - from: "chat_panel.py"
      to: "ChatState"
      via: "state listener"
...
@src/ai/gateway.py (AIGateway with chat_stream method for streaming responses)
...
<task type="auto">
  ...
  <action>
    ...
    NOTE: Use gateway.chat_stream() for streaming chat responses (NOT gateway.generate() which is for single-shot completions).
...
# Verify AIGateway has chat_stream method
assert hasattr(AIGateway, 'chat_stream'), 'AIGateway must have chat_stream method'
print('ChatPanel streaming methods verified')
print('AIGateway.chat_stream method confirmed')
"
    ```
  </verify>
  <done>ChatPanel streams responses from AIGateway using chat_stream method, updates message list incrementally, handles errors gracefully</done>
</task>
...
4. Streaming updates message content incrementally using gateway.chat_stream()
